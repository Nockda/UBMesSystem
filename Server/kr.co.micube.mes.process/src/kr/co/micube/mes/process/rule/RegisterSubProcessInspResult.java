...
public class RegisterSubProcessInspResult extends DatasetRule {
	...
	private IDataTable inspValues; 		// 검사실적 리스트(컬럼 : parameterid, seq, measurevalue)
	...
	// 세부공정 검사실적 삭제 
	private void deleteInspValues(UlSubprocessworkresultData workResult) throws DatabaseException {
		...
		cond.set(UlSubprocessworkresultdetailData.Seq, 0);
	...
	private void insertInspValues(UlSubprocessworkresultData workResult) throws SystemException {
		...
		for (int i = 0; i < this.inspValues.size(); i++) {
			IDataRow insp = this.inspValues.getRow(i);
			CtSubsegmentspecdetailData specDetailData = getSubSegmentDetail(specDetailDataList, insp.getString("PARAMETERID"));
			if(!StringUtils.isNullOrEmpty(insp.getString("MEASUREVALUE"))) {
				UlSubProcessWorkResultDetailService.createSubProcessWorkResultDetail(batch, workResult, specDetailData,
						Constant.ASTERISK, insp.getInteger("SEQ"), insp.getString("MEASUREVALUE"), Constant.NO, this.txnInfo);
			}
		}
		batch.execute();
	}
