...
namespace Micube.SmartMES.Process
{
    ...
    /// 
    /// </summary>
    public partial class AssyDetailEditResult : SmartConditionBaseForm
    {
        ...
        private void InitializeGrid()
        {
            ...
            //스펙 ID
            grdWorkResult.View.AddTextBoxColumn("SPECDEFID", 100).SetTextAlignment(TextAlignment.Center).SetIsReadOnly(true);
            //순서
            grdWorkResult.View.AddTextBoxColumn("USERSEQUENCE", 50).SetLabel("SPECSEQ").SetTextAlignment(TextAlignment.Center).SetIsReadOnly(true);
            //세부공정
            grdWorkResult.View.AddTextBoxColumn("SUBPROCESSSEGMENTNAME", 150).SetIsReadOnly(true);
            //실적입력항목 INPUTRESULTITEM
            grdWorkResult.View.AddTextBoxColumn("PARAMETERNAME", 250).SetLabel("INPUTRESULTITEM").SetIsReadOnly(true);
            // 입력방식
            grdWorkResult.View.AddTextBoxColumn("CODENAME", 90).SetLabel("INPUTTYPE");
            //실적값 RESULTVALUE
            grdWorkResult.View.AddTextBoxColumn("MEASUREVALUE", 90).SetLabel("RESULTVALUE");
            //단위 UNIT
            grdWorkResult.View.AddTextBoxColumn("UNIT", 50).SetIsReadOnly(true);

            //MIN
            grdWorkResult.View.AddTextBoxColumn("LSL", 80).SetLabel("MIN").SetIsReadOnly(true);

            //MAX
            grdWorkResult.View.AddTextBoxColumn("USL", 80).SetLabel("MAX").SetIsReadOnly(true);
            //생성일시
            grdWorkResult.View.AddTextBoxColumn("CREATEDTIME", 140).SetTextAlignment(TextAlignment.Center).SetLabel("WORKDAY").SetIsReadOnly(true);
            //생성자
            grdWorkResult.View.AddTextBoxColumn("CREATOR", 80).SetTextAlignment(TextAlignment.Center).SetLabel("WORKER").SetIsReadOnly(true);

            grdWorkResult.View.AddTextBoxColumn("WORKRESULTHISTKEY", 100).SetIsHidden();
            grdWorkResult.View.AddTextBoxColumn("POINTID", 100).SetIsHidden();
            grdWorkResult.View.AddTextBoxColumn("SEQ", 100).SetIsHidden();
        ...
        public void InitializeEvent()
        {
            ...
            grdInspResult.View.CustomRowCellEdit += View_CustomRowCellEdit;
        ...
        protected override void OnToolbarSaveClick()
        {
            ...
            if (this.tabResult.SelectedTabPageIndex == 1)
            {
                SaveWorkResult();
            }else if(this.tabResult.SelectedTabPageIndex == 2)
            {
                SaveInspResult();
            }
        }

        private void SaveWorkResult()
        {
            this.grdWorkResult.View.PostEditor();

            DataTable changed = grdWorkResult.GetChangedRows();
            ...
            MessageWorker message = new MessageWorker("AssyDetailResult");
        ...
        private void SaveInspResult()
        {
            this.grdInspResult.View.PostEditor();

            DataTable mschanged = grdInspResult.GetChangedRows();

            if (mschanged.Rows.Count == 0)
            {
                // 저장할 데이터가 존재하지 않습니다.
                throw MessageException.Create("NoSaveData");
            }
            ...
            inspectionResult.Columns.Add(new DataColumn("HISTKEY", typeof(string)));
            inspectionResult.Columns.Add(new DataColumn("PARAMETERID", typeof(string)));
            inspectionResult.Columns.Add(new DataColumn("SEQ", typeof(string)));
            inspectionResult.Columns.Add(new DataColumn("MEASUREVALUE", typeof(string)));
            inspectionResult.Columns.Add(new DataColumn("POINTID", typeof(string)));
            ...
            foreach (DataRow row in mschanged.Rows)
            {
                if (string.IsNullOrWhiteSpace(row["CNT"].ToString()) == true
                    || row["CNT"].ToString() == "SPEC")
                    continue;


                foreach (DataColumn col in mschanged.Columns)
                {
                    string colName = col.ColumnName;
                    if (colName.Equals("CNT") || colName.Equals("_STATE_")) continue;
                    ...
                    DataRow chkRow = (from c in inspectionResult.AsEnumerable()
                                      where c["HISTKEY"].ToString() == histKey
                                            && c["PARAMETERID"].ToString() == paramid
                                            && c["POINTID"].ToString() == pointid
                                            && c["SEQ"].ToString() == row["CNT"].ToString()
                                      select c).FirstOrDefault();

                    if (chkRow == null )
                    {
                        newRow = inspectionResult.NewRow();
                        newRow["PARAMETERID"] = paramid;
                        newRow["HISTKEY"] = histKey;
                        newRow["POINTID"] = pointid;

                        newRow["SEQ"] = row["CNT"];
                        //newRow["VALUE"] = row[process + "_" + paramid + "_" + histKey];
                        newRow["MEASUREVALUE"] = row[colName];
                        inspectionResult.Rows.Add(newRow);
                    }
                }
            }
        ...
        protected async override Task OnSearchAsync()
        {
            ...
            int  mainHandle = this.grdMain.View.FocusedRowHandle;
            ...
            if(mainHandle > -1)
            {
                ...
                if (row == null)
                    return;
                this.grdMain.View.FocusedRowHandle = mainHandle;
            }
