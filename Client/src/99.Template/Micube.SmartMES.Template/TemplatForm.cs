...
namespace Micube.Framework.Template
{
    ...
    */
    ...
    public partial class TemplatForm : SmartConditionBaseForm
    {
        ...
        private void GrdGrid_ShowContextMenuEvent(SmartBandedGrid sender, SmartControls.Grid.ShowContextMenuEventArgs args)
        {
        ...
        /// <param name="sender"></param>
        /// <param name="args"></param>
        private void GrdGrid_InitContextMenuEvent(SmartBandedGrid sender, SmartControls.Grid.InitContextMenuEventArgs args)
        {
        ...
        // 신규 행 추가 시 호출 이벤트
        private void View_AddingNewRow(SmartControls.Grid.BandedGrid.SmartBandedGridView sender, SmartControls.Grid.AddNewRowArgs args)
        {
        ...
        /// </summary>
        private void View_AddNewRowChangeControlEvent(SmartControls.Grid.BandedGrid.SmartBandedGridView sender, SmartControls.Grid.BandedGrid.AddNewRowChangeControlArgs args)
        {
        ...
        private void View_GridCellButtonClickEvent(SmartControls.Grid.BandedGrid.SmartBandedGridView sender, SmartControls.Grid.GridCellButtonClickEventArgs args)
        {
        ...
        protected override void InitializeContent()
        {
            ...
            #region ComboBox 설정 예

            SmartComboBox comboBox = new SmartComboBox();
            ...
            //전체 항목 설정
            comboBox.EmptyItemCaption = Language.Get("ALLVIEWS");
            comboBox.EmptyItemValue = "*";

            //DataBinding
            Dictionary<string, object> param = new Dictionary<string, object>();
            param.Add("LANGUAGETYPE", UserInfo.Current.LanguageType);
            
            DataTable dtData = SqlExecuter.Query("쿼리명", "쿼리버전", param);
            //DataTable dtData = SqlExecuter.Procedure("프로시저명", param);
            comboBox.DataSource = dtData;
            comboBox.DisplayMember = "표시될 컬럼명";
            comboBox.ValueMember = "값이될 컬럼명";

            #endregion


            #region SelectPopup 설정 예
            SmartSelectPopupEdit selectPopup = new SmartSelectPopupEdit();

            //SelectPopup에서 사용되는 조건 설정
            ConditionItemSelectPopup selectPopupCondition = new ConditionItemSelectPopup();
            selectPopupCondition.SearchQuery = new SqlQuery("쿼리명", "쿼리버전", $"파라메터");
            selectPopupCondition.ValueFieldName = "값이 될 컬럼명";
            selectPopupCondition.DisplayFieldName = "보여질 컬럼명";
            //Popup창 설정 (창 Title, 버튼, 조회버튼 유무, 창이 열리면서 조회진행 여부)
            selectPopupCondition.SetPopupLayout("Title", PopupButtonStyles.Ok_Cancel, true, true);
            //popup창 크기 및 style 설정
            selectPopupCondition.SetPopupLayoutForm(800, 800, FormBorderStyle.FixedToolWindow);
            //리스트에서 선택 항목 수 (0=무한, 1= 더블클릭하면 항목이 선택되면서 popup 창 close)
            selectPopupCondition.SetPopupResultCount(1);
            //컬럼 자동 너비를 설정
            selectPopupCondition.SetPopupAutoFillColumns("자동너비설정 컬럼명");

            // 조회 조건 설정
            selectPopupCondition.Conditions.AddTextBox("조회조건 다국어 LanguageKey");

            // 조회 결과 Grid Column 설정 (컬럼ID = 다국어 LanguageKey, 컬럼 너비)
            selectPopupCondition.GridColumns.AddTextBoxColumn("컬럼ID", 150);
            selectPopupCondition.GridColumns.AddTextBoxColumn("컬럼ID", 200);
            selectPopupCondition.GridColumns.AddTextBoxColumn("컬럼ID", 60);

            selectPopup.SelectPopupCondition = selectPopupCondition;

            // 값 Clear 버튼 유무 설정
            selectPopup.ClearButtonVisible = false; 
        ...
        private void InitializeGrid_Group()
        {
            ...
            plnatGroup.AddComboBoxColumn("PLANTID", 100, new SqlQuery("GetPlantList", "00001", $"LANGUAGETYPE={UserInfo.Current.LanguageType}"), "PLANTNAME", "PLANTID")
