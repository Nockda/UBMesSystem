    ...
    public static class FrameworkSettings
    {
        ...
        private static void InitializeLanguage()
        {
            ...
            foreach (string serviceId in ServiceList.Keys)
            {
                ...
                language.Merge(SqlExecuter.Query("GetDictionaryList", "10001", new Dictionary<string, object>()
                {
                    { "LANGUAGETYPE", Language.LanguageType },
                    { "VALIDSTATE", "Valid" }
                }));

                message.Merge(SqlExecuter.Query("GetMessageList", "10001", new Dictionary<string, object>()
                {
                    { "LANGUAGETYPE", Language.LanguageType },
                    { "VALIDSTATE", "Valid" }
                }));
