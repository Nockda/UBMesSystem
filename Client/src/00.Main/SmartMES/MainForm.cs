...
namespace SmartMES
{
    public partial class MainForm : XtraForm
    {
        ...
        private const string PRINTER_PREFIX = "프린터 변경 - ";
        ...
        private void InitializeFavorite()
        {
            ...
            foreach (DataRow row in FavoriteMenuList.Rows)
            {
                ...
                DataRow[] rowMenu = _MenuRepository.GetMenuTable().Select("MENUID = '" + menuInfo.MenuId + "'");
        ...
        private void InitializeOptions()
        {
            ...
            CommonFunction.DbLinkName = plantList.Select("PLANTID = '" + UserInfo.Current.Plant + "'").First()["DBLINKNAME"].ToString();
            ...
            foreach (string printer in PrinterSettings.InstalledPrinters)
            {
                ...
                item.Click += (o, e) =>
                {
                    ...
                    // 기본 프린터로 변경
                    SetDefaultPrinter(selectItem.Tag.ToString());

                    //2021-01-18 정송은 '프린트 변경' 다국어 처리 
                    ...
                    this.lblSelectPrinterDescriptionTitle.Text = lblSelectPrinterDescriptionTitle.Text + " - " + selectItem.Tag.ToString();
        ...
        private void InitializeServiceBar()
        {
            ...
            _ServiceMenuCount.ForEach(value =>
            {
                if (value.Value > 0)
                {
        ...
        private void InitializeMenuBar()
        {
            ...
            DataRow[] menuRow = _MenuRepository.GetMenuTable().Select("PARENTMENUID = '' AND MENUTYPE = 'FOLDER'"); // AND VALIDSTATE = 'Valid'");
        ...
        private TileBarDropDownContainer InitializeMenuList(string menuId, string menuName)
        {
            ...
            DataRow[] dataRows = _MenuRepository.GetMenuTable().Select("PARENTMENUID = '" + menuId + "'"); // AND VALIDSTATE = 'Valid'");
            ...
            if (dataRows.Length > 0)
            {
                ...
                for (int i = 0; i < dataRows.Length; i++)
                {
                    ...
                    DataRow[] childRows = _MenuRepository.GetMenuTable().Select("PARENTMENUID = '" + dataRows[i]["MENUID"].ToString() + "'");// AND VALIDSTATE = 'Valid'");
        ...
        private void InitializeEvent()
        {
            ...
            smartDocumentManager1.DocumentActivate += (o, e) =>
            {
                if (e.Document != null)
                {
                    ...
                    lblMenuPath.Text = GetMenuPath(e.Document.Form.Name);
        ...
        private void InitializeLanguageLabel()
        {
            ...
            string strPlantName = dtPlantName.Select("PLANTID = '" + UserInfo.Current.Plant + "'").First()["PLANTNAME"].ToString();

            //2020
            CommonFunction.DbLinkName = dtPlantName.Select("PLANTID = '" + UserInfo.Current.Plant + "'").First()["DBLINKNAME"].ToString();
            ...
            //2021-01-18 정송은 프린트 변경 다국어 처리
            Language.LanguageMessageItem msgChangePrint = Language.GetMessage("CHANGEPRINTER");
            btnSelectPrinter.Text = msgChangePrint.Title ?? msgChangePrint.Message;
            if (UserInfo.Current.IsUseToolTipLanguage)
            {
                ...
                btnSelectPrinter.ToolTip = msgChangePrint.Title ?? msgChangePrint.Message;
            lblSelectPrinterDescriptionTitle.Text = msgChangePrint.Title ?? msgChangePrint.Message;
            lblSelectPrinterDescriptionTitle.Text = lblSelectPrinterDescriptionTitle.Text + " - " + UserInfo.Current.Printer; ;
            lblSelectPrinterDescription.Text = msgChangePrint.Message;
        ...
        private string GetMenuName(string menuId)
        {
            ...
            DataRow[] rowMenu = _MenuRepository.GetMenuTable().Select("MENUID = '" + menuId + "'");
        ...
        private string GetMenuPath(string serviceId, string menuId)
        {
            ...
            DataRow rowCurrent = _MenuRepository.GetMenuTable().Select("MENUID = '" + menuId + "'").First();
            string result = rowCurrent["MENUNAME"].ToString();
            string parentMenuId = rowCurrent["PARENTMENUID"].ToString();

            while (bHasParent)
            {
                DataRow[] rowParent = _MenuRepository.GetMenuTable().Select("MENUID = '" + parentMenuId + "'");
        ...
        private void ChangePlant(string plantId, string plantName)
        {
            Language.LanguageMessageItem msgChangePlant = Language.GetMessage("CHANGEPLANT");
        ...
        private void LoadMainLayoutForDashboardBtn()
        {
            ...
            mainTableLayoutPanel.RowStyles.Add(new RowStyle(SizeType.Percent, (float)40.0));
            mainTableLayoutPanel.RowStyles.Add(new RowStyle(SizeType.Percent, (float)13.0)); // 로고영역
            mainTableLayoutPanel.RowStyles.Add(new RowStyle(SizeType.Percent, (float)5.0));
            mainTableLayoutPanel.RowStyles.Add(new RowStyle(SizeType.Percent, (float)6.0));  // 버튼영역
            mainTableLayoutPanel.RowStyles.Add(new RowStyle(SizeType.Percent, (float)36.0));

            mainTableLayoutPanel.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, (float)15.0));
            mainTableLayoutPanel.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, (float)15.0));
            mainTableLayoutPanel.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, (float)40.0)); // 로고&버튼영역
            mainTableLayoutPanel.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, (float)15.0));
            mainTableLayoutPanel.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, (float)15.0));

            mainTableLayoutPanel.Location = new Point(0, 0);
            mainTableLayoutPanel.Size = new Size(750, 750);
            mainTableLayoutPanel.BackColor = Color.Empty;

           // mainTableLayoutPanel.CellBorderStyle = TableLayoutPanelCellBorderStyle.Outset;
            ...
            btn1.Name = "name";
            //btn1.Text = "종합현황판";
            if (UserInfo.Current.LanguageType == "ko-KR")
                btn1.Text = "종합현황판";
            else
                btn1.Text = "Total Status";
            btn1.Dock = DockStyle.Fill;

            btn1.Font = new Font("Malgun Gothic", 15, FontStyle.Bold);
            btn1.BackColor = Color.FromArgb(0, 125, 198);
            btn1.ForeColor = Color.White;
            btn1.FlatStyle = FlatStyle.Flat;

            PictureBox pic = new PictureBox();

            pic.Image = Properties.Resources.ULVAC_main_logo;
      
            pic.SizeMode = PictureBoxSizeMode.StretchImage;
            pic.Dock = DockStyle.Fill;

            TableLayoutPanel btnTableLayouyPanel = new TableLayoutPanel();

            btnTableLayouyPanel.ColumnCount = 3;

            btnTableLayouyPanel.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, (float)30.0));
            btnTableLayouyPanel.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, (float)40.0));
            btnTableLayouyPanel.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, (float)30.0));

            btnTableLayouyPanel.BackColor = Color.Empty;

            btnTableLayouyPanel.Dock = DockStyle.Fill;

            //btnTableLayouyPanel.CellBorderStyle = TableLayoutPanelCellBorderStyle.Outset;

            btnTableLayouyPanel.Controls.Add(btn1, 1, 0);

            mainTableLayoutPanel.Controls.Add(btnTableLayouyPanel, 2, 3);
            mainTableLayoutPanel.Controls.Add(pic, 2, 1);
        ...
        private void Btn1_Click(object sender, EventArgs e)
        {
            ...
            Micube.SmartMES.DashBoard.TotalDashBoardStatusPop pop = new Micube.SmartMES.DashBoard.TotalDashBoardStatusPop(LanguageType);
